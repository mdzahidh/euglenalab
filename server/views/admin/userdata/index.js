...
exports.find = function(req, res, next){
  ...
  req.app.db.models.User.pagedFind({
    ...
    if (req.xhr) {
      ...
      results.filters = req.query;
      res.render('admin/userdata/index', { data: { results: JSON.stringify(results) } });
    }
  });
};
...
exports.read = function(req, res, next) {
  req.app.db.models.User.findById(req.params.id).populate('roles.admin', 'name.full').populate('roles.account', 'name.full').exec(function(err, user) {
    ...
    if(req.xhr) {res.send(user);
    } else {
      req.app.db.models.Experiment.find({'user.name':user.username}, {}, function(err, experiments) {
        res.render('admin/userdata/details', { 
          data: { 
            record: escape(JSON.stringify(user)),
            experiments: escape(JSON.stringify(experiments)),
          } 
        });
      });
    }
