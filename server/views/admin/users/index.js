...
exports.find = function(req, res, next) {
  ...
  req.app.db.models.User.pagedFind({
    filters: filters,
    keys: 'username email isActive groups allGroups',
...
exports.read = function(req, res, next){
  ...
  var getGroupNames=function(cb_fn) {
    req.app.db.models.Group.find({}, {}, '', function(err, groups) {
      if(err) {
        cb_fn(err);
      } else {
        ...
        groups.forEach(function(grp) {
          console.log(grp.name, grpsTemp[grp.name]);
          if(grpsTemp[grp.name]===null || grpsTemp[grp.name]===undefined) {
            grpsTemp[grp.name]=grp.name;
            outcome.groupsArray.push(grp.name);
          }
        });
        cb_fn(null);
      }
  ...
  getGroupNames(function(err) {
    console.log(outcome.groupsArray);
    req.app.db.models.User.findById(req.params.id).populate('roles.admin', 'name.full').populate('roles.account', 'name.full').exec(function(err, user) {
      ...
      if(outcome.groupsArray.forEach && outcome.groupsArray.length>0) {
        user.allGroups=outcome.groupsArray;
        user.save();
      }
      if (req.xhr) {
        res.send(user);
...
exports.update = function(req, res, next){
  ...
  workflow.on('patchUser', function() {
    req.app.db.models.Group.getAllGroups(function(err, allGroups) {
      var parts=req.body.groups.split(',');
      var userGroups=[];
      parts.forEach(function(part) {
        if(part.length>0) {
          allGroups.forEach(function(grp) {
            if(part.trim().toLowerCase()===grp.trim().toLowerCase()) {
              userGroups.push(grp.trim().toLowerCase());
            }
          });
        }
      });
      console.log(userGroups, allGroups);
      var fieldsToSet = {
      ...
      req.app.db.models.User.findByIdAndUpdate(req.params.id, fieldsToSet, {new:true}, function(err, user) {
        if (err) {
          return workflow.emit('exception', err);
        }
