...
exports.find = function(req, res, next) {
  ...
  var getSession=function(callback) {
    var sessUpdate={
      ...
      socketID:null, socketHashID:null, socketHandle:null,
      user:{
        id:null,
        name:null,
        groups:['golabz']
  ...
  var getUser=function(callback) {
    if(outcome.hasUser) {
      ...
      outcome.user={
        groups:outcome.session.user.groups
      };
  ...
  async.series(initSeriesFuncs, function(err) {
    if(err) {
      ...
      if(outcome.data.user.name===null || outcome.data.user.name===undefined ||
        outcome.data.user.id===null || outcome.data.user.id===undefined) {
        outcome.data.results={none:['none']};
        outcome.data.hasResults=false;
        outcome.data.notOnlyLive=true;
        res.render('basicuser/index', {data:outcome.data});
      } else {
        res.render('basicuser/index', {data:outcome.data});
      }
    }
...
exports.download = function(req, res, next) {
  ...
  req.app.db.models.BpuExperiment.findById(req.params.id, {}, function(err, userExp) {
    ...
    } else if(userExp.user_tarFilePath && userExp.user_tarFilename) {
...
exports.read = function(req, res, next) {
  req.app.db.models.BpuExperiment.findById(req.params.id).populate('roles.admin', 'name.full').populate('roles.account', 'name.full').exec(function(err, userExp) {
    ...
    if (req.xhr) {
      ...
      res.render('basicuser/details', { data: { record: escape(JSON.stringify(userExp)) } });
    }
  });
};
