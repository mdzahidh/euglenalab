/* global app:true */
var doPrint=true;
var myPrint=function(msg) {if(doPrint) console.log('(public'+'/admin/bpus/index.js)'+'\t'+msg);};
(function() {
  'use strict';
  ...
  app.Record = Backbone.Model.extend({
    ...
    url: function() {
      return '/admin/bpus/'+ (this.isNew() ? '' : this.id +'/');
    }
  });
  ...
  app.Router = Backbone.Router.extend({
    ...
    initialize: function() {
      ...
      app.mainView.mySocket=io.connect();
      app.mainView.mySocket.on('connect', function() {
        myPrint('connect');
        app.mainView.mySocket.emit('/admin_bpus/#setConnection', {});
      });
      app.mainView.mySocket.on('/admin_bpus/#setConnectionRes', function() {
        myPrint('/admin_bpus/#setConnectionRes');
        app.mainView.mySocket.emit('/admin_bpus/#ping', {});
      });
      app.mainView.mySocket.on('/admin_bpus/#pingRes', function() {
        myPrint('/admin_bpus/#pingRes');
      });
    },
    default: function() {
      if (!app.firstLoad) {
