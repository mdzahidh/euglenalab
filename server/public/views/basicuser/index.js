...
//user bpu experimentType tarFilename tarFileLocation startRunTime textExpFilename
  app.Record = Backbone.Model.extend({
    ...
    urlDownload: function() {
      return '/basicuser/download/'+ (this.isNew() ? '' : this.id +'/');
    },
    url: function() {
      return '/basicuser/'+ (this.isNew() ? '' : this.id +'/');
    }
  });
  ...
  app.MainView = Backbone.View.extend({
    ...
    submitExperimentFromViews:function(type, wantsBpuName) {
      ...
      if(app.userSocketClient.isInitialized && !app.mainView.userExpInfo.isSubmitting) {
        ...
        if(type==='live') {
          ...
          joinQueueData.user.groups=app.mainView.session.attributes.user.groups;
            ...
            if(err) {
              ...
              if(validationObjs && validationObjs.forEach) {
                ...
                validationObjs.forEach(function(validationObj) {
                  cnt++;
                  console.log('4-'+cnt+'. submitExperimentFromViews', validationObj.group_experimentType, validationObj.exp_wantsBpuName, validationObj.exp_eventsRunTime, validationObj.isValid);
                });
                setTimeout(function() {
                  console.log('5. reset. submitExperimentFromViews', 'app.mainView.userExpInfo.isSubmitting is now false again');
                  app.mainView.userExpInfo.isSubmitting=false;
    ...
    disableUI:function(caller) {
      ...
      if(app.textSubmitView) app.textSubmitView.disableSubmitTextNextButton(true, caller+'+'+'app.mainView.disableUI');
      if(app.bpuImageView) app.bpuImageView.disableAll(true, caller+'+'+'app.mainView.disableUI');
    },
    ...
    updateJoinQueueSequence:function(msg) {
      ...
      //Text
      if(app.textSubmitView) app.textSubmitView.setSubmitTextNextLabel('Text Submit: '+'Submitting Exp(s) status: '+msg);
    },
    ...
    updateFromServer:function(updateObj) {
      ...
      if(updateObj.bpusPackage && updateObj.bpusPackage.forEach && updateObj.bpusPackage.length>0) {
        updateObj.bpusPackage.forEach(function(bpuPack) {
          if(bpuPack.liveBpuExperiment) {
            //Title
            if(app.bpuImageView) app.bpuImageView.setTitleLabel(bpuPack.index, bpuPack.name+', User:'+bpuPack.liveBpuExperiment.username);
            //User
            var secondsLeft=Math.round(bpuPack.liveBpuExperiment.bc_timeLeft/1000);
            if(app.bpuImageView) app.bpuImageView.setUserLabel(bpuPack.index, 'Time Left:'+secondsLeft+' seconds');
            //Status
            if(app.bpuImageView) app.bpuImageView.setStatusLabel(bpuPack.index, 'Status:'+bpuPack.bpuStatus);
          } else {
            //Title
            if(app.bpuImageView) app.bpuImageView.setTitleLabel(bpuPack.index, bpuPack.name+', User:'+'None');
            //User
            if(app.bpuImageView) app.bpuImageView.setUserLabel(bpuPack.index, 'Time Left:'+0+' seconds');
            //Status
            if(app.bpuImageView) app.bpuImageView.setStatusLabel(bpuPack.index, 'Status:'+bpuPack.bpuStatus);
          }
        });
        ...
        app.mainView.bpus.forEach(function(bpu) {
          //Title
          if(app.bpuImageView) app.bpuImageView.setTitleLabel(bpu.index, 'eug'+bpu.index+', User:'+'None');
          //User
          if(app.bpuImageView) app.bpuImageView.setUserLabel(bpu.index, 'Time Left:'+'0'+' seconds');
          //Status
          if(app.bpuImageView) app.bpuImageView.setStatusLabel(bpu.index, 'Status:'+'Unknown');
        });
      }
      ...
      if(!app.mainView.userExpInfo.isSubmitting) {
        ...
        if(app.textSubmitView) app.textSubmitView.setSubmitTextNextLabel(textMsg);
        ...
        if(app.textSubmitView) app.textSubmitView.disableSubmitTextNextButton(isTextDisabled, 'app.mainView.server update');

        //Set text/live join button for bpu view
        app.mainView.bpus.forEach(function(bpu) {
          if(app.bpuImageView) app.bpuImageView.disableLiveButton(bpu.index, isLiveDisabled, 'app.mainView.server update');
          if(app.bpuImageView) app.bpuImageView.disableTextButton(bpu.index, isTextDisabled, 'app.mainView.server update');
        });
      }
    ...
    initialize: function() {
      ...
      app.mainView.hasResults=false;//hardcode, could pass them in
      app.mainView.notOnlyLive=false;//hardcode, could pass them in
