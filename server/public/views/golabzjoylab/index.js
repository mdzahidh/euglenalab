...
(function() {
  ...
  app.MainView=Backbone.View.extend({
    ...
    getLedsSetObj:function() {
      if(app.mainView.ledsSetObj===null) return null;
      else return JSON.parse(JSON.stringify(app.mainView.ledsSetObj));
    }, 
    ...
    setLedsFromEvent:function(evt, evtType, previousTouchState) {
      ...
      if((newDate-app.mainView.lastSetLedsFromEventDate)>=app.mainView.setLedsFromEventInterval) {
        ...
        var ledsSetObj=app.mainView.getLedsSetObj(); 
        ledsSetObj.metaData.clientTime=app.mainView.lastSetLedsFromEventDate.getTime();
        ...
        ledsSetObj=app.mainView.setLedsFromObjectAndSendToServer(ledsSetObj, previousTouchState+'->setLedsFromEvent');
      }
    },
    ...
    initialize: function() {
      ...
      var socketOptions={
        ...
        connectionInfo:{
          ...
          sessionObjID:app.mainView.session.id,
          sessionID:app.mainView.session.get('sessionID'),
          liveBpuExperimentID:app.mainView.bpuExp.id,
          socketID:null, socketHandle:null,
        }
      };
      app.userSocketClient.setConnection(socketOptions, function(err, setLedsObj) {
        if(err) {
          app.mainView.isSocketInitialized=false;
          app.mainView.kickUser(err);
        } else {
          console.log('setConnection setLedsObj', setLedsObj);
          app.userSocketClient.startUpdateBpus();
          app.mainView.ledsSetObj=setLedsObj;
          //My Objects 
    ...
    startUpdateLoop:function() {
      ...
      app.mainView.updateLoopInterval=setInterval(function () {
        ...
        if(app.mainView.timeInLab>(1.05*app.mainView.timeForLab)) {
          app.mainView.kickUser();
        } else {
