...
#define PORT    "32000" /* Port to listen on */
...
{
    OPRINT("cleaning up ressources allocated by socket listener thread\n");
...
void* server(void *whatever)
{
    ...
    while(1) {
        ...
        if (newsock == -1) {
            ...
            if((err = recv(newsock,buffer,BUFFERSIZE,0)) < 0 ){
                perror("RECV encountered error");
            }
            else if( err > 0 ){
                printf("Command received: %s\n",buffer);
                if( strcmp(buffer, "start") == 0 ){
                    pthread_mutex_lock(&save_mutex);
                    printf("Starting to save images\n");
                    save_pics = 1;
...
void help(void)
{
    fprintf(stderr, " ---------------------------------------------------------------\n" \
            ...
            " [-p | --port ] .........: port for the control for saving files, send \"start\"/\"stop\" to this port\n"\
            " ---------------------------------------------------------------\n");
}
...
void *worker_thread(void *arg)
{
    ...
    while(ok >= 0 && !pglobal->stop) {
        ...
        pthread_mutex_lock(&save_mutex);
        while( !save_pics ){
            printf("Thread going to a sleep\n");
            pthread_cond_wait(&save_cond,&save_mutex);
            printf("Thread waking up to start saving\n");
        }
        pthread_mutex_unlock(&save_mutex);
        ...
        if (mjpgFileName == NULL) { // single files with ringbuffer mode
            ...
            if(strftime(buffer1, sizeof(buffer1), "%%s/%s%%03llu.jpg", localtime(&curTime.tv_sec)) == 0) {
            ...
            snprintf(buffer2, sizeof(buffer2), buffer1, folder, milli);
